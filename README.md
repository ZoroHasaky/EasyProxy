...
```bash
git clone https://github.com/yourname/ezproxy.git && cd ezproxy
docker compose -f deploy/docker-compose.yml up -d --build
# 面板: http://<host>:8080   初始密码: docker logs ezproxy
...
1. `docker logs ezproxy` 获取初始密码，登录并按提示修改
...
# 后端（单二进制，内嵌前端）
cd backend && go build -ldflags "-X main.version=$(cat ../VERSION)" -o ezproxy ./cmd/server
# Docker
docker build -f deploy/Dockerfile -t ezproxy .
```
...
- 自更新需要设置页配置更新源仓库（owner/repo）且 Release 中包含对应架构的 `ezproxy-linux-<arch>.tar.gz`
