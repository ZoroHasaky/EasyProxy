...
进入面板“内核”页，开启“多端口分流”后，可以为 7890 及新增的混合端口分别配置识别规则和出站目标。关闭开关时，额外端口配置会保留，但不会被 Mihomo 监听。

普通 Docker bridge 模式不会自动暴露新增端口。若新增了 7891，请在 Compose 的服务端口中手动增加：
...
使用 `network_mode: host` 时不需要增加 Compose 端口映射。

端口规则会按“识别规则”的优先级从高到低匹配；端口未命中任何启用规则时，使用该端口设置的默认出口。每个端口的规则只影响从该端口进入的代理流量，TUN/透明代理仍使用全局规则。
